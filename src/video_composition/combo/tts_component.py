...
class TTSComponent(ComponentContainer):
    def __init__(self, text, width=1080, height=1920, fps=30, color=(0, 0, 0), speaker_name="p225", *args, speed=1.0, start = 0, **kwargs):
        super().__init__(*args, **kwargs)
        self.text = text
        self.words = TextToSpeech.custom_split(text)

        output = TextToSpeech.text_to_audio(text, speed=speed, speaker_name=speaker_name)
